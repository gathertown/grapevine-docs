...
The Grapevine MCP provides 3 categories of tools that can be used to query your Company Context:
...
[Data Warehouse Tools](#data-warehouse-tools) allow you to query structured data warehouses like Snowflake using natural language or direct SQL for analytical queries. 
...
<Accordion title="ask_data_warehouse" icon="database">

Query structured data warehouses using natural language. The tool translates your question into SQL using AI (e.g., Snowflake Cortex Analyst) and executes it against the data warehouse.
...
<Tabs>
  <Tab title="Request">
    <ParamField path="question" type="string" required>
      Natural language question to query the data warehouse
    ...
    <ParamField path="source" type="string" default="snowflake">
      Data warehouse source. Currently supported: `snowflake`
    </ParamField>

    <ParamField path="semantic_model_id" type="string">
  ...
  <Tab title="Response">
    ...
    <ResponseField name="results" type="object[]">
      Array of result rows, each row is an object mapping column names to values
    </ResponseField>

    <ResponseField name="generated_sql" type="string">
      The SQL query that was generated and executed
    </ResponseField>

    <ResponseField name="explanation" type="string">
      Optional explanation from the data warehouse's AI about the query
    </ResponseField>
...
<Accordion title="execute_data_warehouse_sql" icon="code">
...
<Tabs>
  ...
  <Tab title="Response">
    Returns a formatted markdown string with query results displayed as a table.

    **Security:**
    - Uses user's OAuth token for authentication
    - Respects native data warehouse permissions (RBAC, row-level security)
